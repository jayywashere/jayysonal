...
export function initBgm(): void {
    ...
    bgm.addEventListener("error", () => {
        console.error(
            `[Error] Could not load audio: ${bgm.src} (media error code: ${bgm.error?.code ?? "unknown"})`,
        );
    });
    ...
    const disc = document.querySelector<HTMLImageElement>("#disc");
    const nowPlayingLabel =
        document.querySelector<HTMLParagraphElement>("#now-playing");

    const updateDisplayLabel = (text: string): void => {
        if (nowPlayingLabel) {
    ...
    bgm.addEventListener("ended", () => {
        ...
        bgm.play().catch((err) => {
            console.error(
                `[Error] The playlist track cycle was blocked: ${err}`,
            );
        });
    });
